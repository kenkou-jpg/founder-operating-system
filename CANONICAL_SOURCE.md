...
| **Founder OS 成熟度・使用状況** | **`analytics-os/os-metrics/CURRENT_STATE.md`** | **PR完了・週次** |
| ポートフォリオKPI | `portfolio-os/PORTFOLIO_KPI.md` | 月次 |
| 法務方針 | `legal-os/README.md` | 随時 |
| 重要意思決定の記録 | `governance/DECISION_LOG.md` | 随時 |
| **Founder 判断履歴（Workflow OS）** | **`founder-workflow-os/09_DECISION_LOG.md`** | **随時** |
| **プロジェクト現在地管理** | **`founder-workflow-os/08_PROGRESS_REGISTRY.md`** | **PR完了・Stage変更時** |
...
```
**Progress Registry と Decision Log の権限について:**
- `founder-workflow-os/08_PROGRESS_REGISTRY.md` は現在地管理の正。ただし Binding Authority は持たない。
- `founder-workflow-os/09_DECISION_LOG.md` は Founder 判断履歴の正。ただし Governing Documents より下位であり、直接的な設計変更権限を持たない。
- 両文書は記録・参照目的であり、IPPO を含む各プロダクトの Governing Documents を上書きしない。
...
### Canonical Source が「状態管理」か「設計権限」かの判断

| 情報の種類 | 正しい置き場所 | 権限 |
|-----------|-------------|------|
| 現在どのPR・ステージにいるか | `founder-workflow-os/08_PROGRESS_REGISTRY.md` | 記録のみ（設計変更権限なし） |
| Founder の判断とその理由 | `founder-workflow-os/09_DECISION_LOG.md` | 記録のみ（Binding Authority なし） |
| 拘束力のある決定 | `governance/BINDING_DECISIONS.md` | Binding Authority あり |
| IPPOのアーキテクチャ | ippo リポジトリ `docs/ARCHITECTURE.md` | プロダクト Governing Document |
