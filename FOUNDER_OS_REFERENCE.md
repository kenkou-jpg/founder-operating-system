...
> BOOTSTRAP.md → このファイル → Execution Dispatcher の順に読む。
> Claude Code は Founder OS を探してはいけない。必ず BOOTSTRAP.md を起点として、このファイルで参照先を確定する。
...
| **App Runtime Export** | 同上 `development-os/app-runtime/` | `master` | App Runtime Generation — 各アプリ向け軽量 Runtime の生成・テンプレート管理 |
...
Claude Code は Founder Operating System を検索してはいけない。
必ず以下の順序で実行を開始する。

```
BOOTSTRAP.md（実行起点）
  ↓
FOUNDER_OS_REFERENCE.md（このファイル — 参照先を確定）
  ↓
Execution Dispatcher → MODE_SELECTION_MATRIX → Optimization Pack → Implementation
```

**Repository 探索は禁止。推測によるパス指定は禁止。**

---

## Founder OS Invocation

「Founder Operating System に従ってください。」というプロンプトは以下と同義。
