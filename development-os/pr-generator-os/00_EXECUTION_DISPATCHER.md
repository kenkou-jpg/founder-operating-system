...
## Repository Exploration Policy（必須・v1.0 追加）

```
Execution Mode 決定直後、SMART_DOCUMENT_LOADING.md を読む前に、
必ず REPOSITORY_EXPLORATION_POLICY.md を確認し探索範囲を確定する。
...
STANDARD_MODE / FAST_MODE では以下を禁止する（詳細: `REPOSITORY_EXPLORATION_POLICY.md`）。
...
```
✅ HANDOFF / Roadmap 該当PRエントリ / PR Scope
✅ 直接依存ファイル（HANDOFF / Responsibility Registry に明記されたもの）
✅ 直接関連テスト
✅ 必須 Validation 文書（Responsibility / Roadmap / Scope / BD Compliance）
```

FULL_MODE の必須 Validation・必須読込（Architecture 全体・Council 文書等）はこのポリシーで制限されない。
...
Execution Dispatcher（このファイル）を読む
  ↓
  【再読禁止】上記 + 以下は Startup Sequence で既読。この時点で再読してはいけない:
    CLAUDE.md / BOOTSTRAP / FOUNDER_OS_REFERENCE / MODE_SELECTION_MATRIX /
    REPOSITORY_EXPLORATION_POLICY / SMART_DOCUMENT_LOADING /
    TOKEN_OPTIMIZATION / REPORT_OPTIMIZATION
  【次に読む PR Context（Startup Sequence に含まれない）】:
    HANDOFF / PR_INPUT_SHEET
  ↓
...
Execution Metadata を生成（必須）
  ↓
REPOSITORY_EXPLORATION_POLICY.md 確認（探索範囲確定・Background Agent禁止）
  ↓
Validation（Responsibility → Roadmap → Scope → BD Compliance）
  ↓
Conditional Loading 判定（Architecture / Decision Log / Progress Registry / Analytics）
  ↓
必要文書のみ追加読込（条件不成立の文書は読まない）
  ↓
Implementation
  ↓
Completion Report（REPORT_OPTIMIZATION.md に従い Mode 別短縮）
  ↓
Progress Registry Live Progress 更新（Execution Metadata を転記）
  ↓
...
```
1. REPOSITORY_EXPLORATION_POLICY.md ← 探索範囲の確定（Background Agent / 全探索禁止）
2. SMART_DOCUMENT_LOADING.md  ← 読む文書の絞り込みルール
3. TOKEN_OPTIMIZATION.md      ← トークン消費の行動規範
4. 選択した MODE.md           ← 必須チェックリスト
5. REPORT_OPTIMIZATION.md     ← Completion Report の出力形式
```
...
- `REPOSITORY_EXPLORATION_POLICY.md` — Repository 探索制限ルール（v1.0）★Mode 決定直後に確認
